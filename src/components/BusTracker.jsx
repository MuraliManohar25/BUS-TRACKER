...
<<<<<<< HEAD
      ...
      // Get location directly using geolocation API
...
<<<<<<< HEAD
        ...
        try {
          ...
          if (errorMsg.includes('permission') || errorMsg.includes('Permission') || errorMsg.includes('Missing or insufficient')) {
            setLocationError('Permission denied. Please check Firestore rules are deployed or enable Firestore database.');
          } else {
...
<<<<<<< HEAD
                <button
                  type="button"
                  onClick={() => {
                    console.log('Toggle button clicked, current state:', isBeaconActive);
                    if (selectedBus) {
                      handleBeaconToggle(!isBeaconActive);
                    } else {
                      setLocationError('Please select a bus first.');
                    }
                  }}
                  disabled={!selectedBus}
                  className={`relative flex h-8 w-14 items-center rounded-full border-none p-1 transition-all duration-200 ${
                    isBeaconActive 
                      ? 'bg-blue-600 justify-end' 
                      : 'bg-slate-200 dark:bg-slate-600 justify-start'
                  } ${!selectedBus ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:opacity-80 active:scale-95'}`}
                  aria-label={isBeaconActive ? 'Stop beacon tracking' : 'Start beacon tracking'}
                  aria-pressed={isBeaconActive}
                >
                  <div className={`h-6 w-6 rounded-full bg-white shadow-sm transition-all duration-200 ${
